...
def verify_sphere_packing(sphere_centers: np.ndarray, tol: float = 1e-6):
    ...
    return min_squared_distance, max_squared_norm
...
def evaluate(program_path: str):
    try:
        ...
        packing_quality_ratio = max_squared_norm / min_squared_distance if min_squared_distance > 0 else 1
        bonus_ratio = 1 / BENCHMARK * np.exp(-packing_quality_ratio)
        benchmark_ratio += bonus_ratio
